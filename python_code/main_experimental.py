"""
main_experimental.py — SDC with traffic-sign brake control (experimental).

Stop sign  → full brake, hold 10 s, then resume.
Red light  → full brake, hold until green light detected, then resume.

Adjust STOP_LABELS / RED_LABELS / GREEN_LABELS to match your model's class names.
"""
...
# Adjust these to match your model's class names (lowercased for comparison).
STOP_LABELS  = {"stop-sign"}
RED_LABELS   = {"red"}
GREEN_LABELS = {"green"}

STOP_HOLD_DURATION = 10.0   # seconds to wait at a stop sign
BRAKE_RAMP_DURATION = 1.0   # seconds spent ramping brake to full before marking stopped
DETECTION_CONFIDENCE = 0.5  # minimum confidence to act on a detection
...
class TrafficStateController:
    """
    Integrates with detections each frame.
    Call update() every frame; it returns whether braking is active,
    the state name, and a display string.
    """

    def __init__(self):
        self.state        = DriveState.DRIVING
        self.state_start  = time.time()
        self._brake_cause = None   # "sign" | "red"
    ...
    def _transition(self, new_state, kart, reason=""):
        ...
        if new_state == DriveState.BRAKING:
            ...
            if kart:
                kart.brake(1.0)      # maintain full brake
        elif new_state == DriveState.RESUMING:
            if kart:
                kart.release()
        # DRIVING: brake already released by RESUMING
    ...
    def update(self, detections, kart):
        ...
        Call once per frame.
        Returns:
            brake_active (bool)  – True when kart should not steer/drive
            state_label  (str)   – current state name for display
            status_text  (str)   – detailed status with countdown
        """
        now     = time.time()
        ...
        status_text = self._status_text(elapsed)
        return brake_active, self.state.value, status_text

    def _status_text(self, elapsed):
        if self.state == DriveState.STOPPED_SIGN:
            remaining = max(0.0, STOP_HOLD_DURATION - elapsed)
            return f"STOP SIGN — resuming in {remaining:.1f}s"
...
# Colour map for traffic state overlay banner
_STATE_COLOURS = {
    DriveState.DRIVING:      (0,   200,   0),
    DriveState.BRAKING:      (0,   165, 255),   # orange
    DriveState.STOPPED_SIGN: (0,     0, 220),   # red
...
def build_display(base_frame, boxes_frame, left_line, right_line,
    ...
    cv2.rectangle(display, (0, 110), (600, 145), (30, 30, 30), -1)
    cv2.putText(display, f"Traffic: {traffic_text}",
                (10, 138), font, 0.9, colour, 2, cv2.LINE_AA)
...
def run_on_camera(source=1):
    ...
    print("Camera mode (experimental) — press 'q' to quit, 'e' for e-stop.")
...
def run_on_video(path):
    ...
    cv2.namedWindow("SDC View [EXP]", cv2.WINDOW_NORMAL)
    cv2.resizeWindow("SDC View [EXP]", 1280, 720)
    print("Video mode (experimental) — press 'q' to quit, 'p' pause/resume, 'e' e-stop.")
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage:")
        print("  python -m python_code.main_experimental camera")
        print("  python -m python_code.main_experimental camera 1")
        print("  python -m python_code.main_experimental video test.mp4")
